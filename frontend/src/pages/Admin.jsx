    ...
    <div style={{ ...s.card, overflow: 'hidden' }}>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid #f2f2f7', padding: '16px' }}>
          ...
          {/* Verificatielaag */}
          {!verified && <VerificatieGate machine={m} onVerified={() => setVerified(true)} />}
...
function MachineZoekenTab() {
  ...
  const Dot = ({ online }) => (
    <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: online ? '#34c759' : '#c7c7cc', marginRight: 6 }} />
  )
    ...
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <input
          ...
          placeholder="Zoek op short code, naam of serienummer…"
          style={{ ...s.inp, flex: 1 }}
        />
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {updateMsg && (
            <div style={{ padding: '10px 14px', borderRadius: 10, marginBottom: 12, fontSize: 13,
              background: updateMsg.ok ? '#f0faf3' : '#fff5f5',
              color: updateMsg.ok ? '#1a7a3a' : '#cc2200',
              border: `1px solid ${updateMsg.ok ? '#a3e6b4' : '#ffb8b8'}` }}>
              {updateMsg.text}
            </div>
          )}
